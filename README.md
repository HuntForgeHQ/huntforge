...
HuntForge follows a layered modular architecture with clear separation of concerns.
...
│          ▼             ▼             ▼                            │
│    ┌──────────┐ ┌──────────┐ ┌──────────┐                       │
│    │Tag       │ │HFLogger  │ │Resource  │                       │
│    │Manager   │ │• Scan    │ │Monitor   │                       │
│    │• Store   │ │  events  │ │• RAM/CPU │                       │
│    │  tags    │ │• Phases  │ │• Load     │                       │
│    │• Query   │ │• Tools   │ │• Activity│                       │
│    │  (has_tag)│• Tags    │ │          │                       │
│    └──────────┘ └──────────┘ └────┬─────┘                       │
│                                    │                               │
│                                    ▼                               │
...
│               ▼                    │                    ▼         │
│    ┌──────────────────┐           │           ┌──────────────────┐│
│    │  Docker Runner   │◄──────────┘           │ Checkpoint       ││
│    │  • exec in ctn   │                       │  • Save state    ││
│    │  • Stream logs   │                       │  • Resume scan   ││
│    └──────────────────┘                       └──────────────────┘│
│                                    │                               │
│                         ┌──────────▼──────────┐                   │
│                         │   Docker Container   │                   │
...
│                         ORCHESTRATOR V2                         │
├─────────────┬─────────────┬──────────────┬──────────────────────┤
│ Tag Manager │  Scheduler   │  HFLogger   │  Checkpoint Manager │
├─────────────┼─────────────┼──────────────┼──────────────────────┤
│ • has_tag() │ • can_sched?│ • event:     │ • save_checkpoint() │
│ • add()     │ • get_load()│   START      │ • load_checkpoint() │
│ • get_all() │ • register_ │ • event:     │                      │
│             │   start()   │   COMPLETE   │                      │
│             │ • register_ │ • event:     │                      │
│             │   end()     │   SKIP       │                      │
└─────────────┴─────────────┴──────────────┴──────────────────────┘
         ...
         │                │               │
         ▼                ▼               ▼
    Tags persist   Timing/metrics   Scan state
    across phases  in logs          for resume
```
