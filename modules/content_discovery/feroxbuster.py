...
class FeroxbusterModule(BaseModule):
    def build_command(self, target: str, container_out: str) -> list:
        return ['feroxbuster', '--url', f'https://{target}', '--output', container_out, '--quiet', '--auto-tune']

    def run(self, target: str, output_dir: str, tag_manager, config: dict = None) -> dict:
        self.config = config or {}
        
        host_out = os.path.join(output_dir, 'raw', 'feroxbuster.txt')
        ...
        return {'results': urls, 'count': len(urls), 'requests_made': 5000}

    def emit_tags(self, result: dict, tag_manager) -> None:
        if result['count'] > 0:
            tag_manager.add('hidden_paths_found', confidence='high', source='feroxbuster')
